...
export function ImageUploader({
  ...
  async function uploadOne(file: File) {
    // 1) ask the gateway for a presigned PUT
    const signed = await api.presignImage({
    ...
    // 2) upload directly to S3 (LocalStack in dev) with XHR for progress
    ...
    // 3) tell the service the upload is done — flips status to 'ready'
    const confirmed = await api.confirmImage(signed.id);
    return { id: signed.id, url: confirmed.url, key: signed.key };
  }
